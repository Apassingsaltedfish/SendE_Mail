...
bool CSmtp::CreateConn()
{
	...
	if (connect(skClientTemp, (sockaddr*)& saddr, sizeof(saddr)))	return false;
...
{
	if ((int)send(sockClient, strMessage.c_str(), (int)strMessage.length(), 0) < 0)	return false;
...
int CSmtp::Login()
{
	...
	sendBuff += user;
	sendBuff += "\r\n";
	if (!Send(sendBuff) ||!Recv())	return 1;
	sendBuff.empty();
	sendBuff = "AUTH LOGIN\r\n";
	if (!Send(sendBuff) ||!Recv())	return 1;
	sendBuff.empty();
	...
	delete[]ecode;
	if (!Send(sendBuff) ||!Recv())	return 1;
	if (strstr(buff, "550"))		return 2;
	if (strstr(buff, "535"))		return 3;
	return 0;
}
...
bool CSmtp::SendEmailHead()
{
	...
	if (!Send(sendBuff) ||!Recv())	return false;
	sendBuff.empty();
...
{
	if (!CreateConn())		return 1;
	if (Login())			return Login();
	if (!SendEmailHead())	return 1;
	if (!SendTextBody())	return 1;
	if (!SendEnd())			return 1;
	return 0;
}
